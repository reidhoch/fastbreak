"""Player Dashboard by Team Performance endpoint for analyzing player stats by team performance."""
...
class PlayerDashboardByTeamPerformance(
    PlayerDashboardEndpoint[PlayerDashboardByTeamPerformanceResponse]
):
    """Fetch player stats split by team performance metrics.

    Returns player statistics grouped by score differential, points scored,
    and points against.
    ...
    path: ClassVar[str] = "playerdashboardbyteamperformance"
    response_model: ClassVar[type[PlayerDashboardByTeamPerformanceResponse]] = (
        PlayerDashboardByTeamPerformanceResponse
    )
