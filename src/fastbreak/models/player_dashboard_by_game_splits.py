"""Models for the Player Dashboard by Game Splits endpoint response."""
...
class GameSplitStats(PandasMixin, PolarsMixin, BaseModel):
    """Player statistics for a game split segment.

    Contains aggregated stats and league ranks for a specific game segment
    (half, period, score margin, etc.).
    """
...
class PlayerDashboardByGameSplitsResponse(FrozenResponse):
    ...
    - by_half: Stats by half (First Half, Second Half, Overtime)
    - by_period: Stats by period (1, 2, 3, 4, OT1, etc.)
    - by_score_margin: Stats by general score margin (Tied, Behind, Ahead)
    - by_actual_margin: Stats by specific score margin ranges
    """
        ...
        named_result_sets_validator(
            {
                ...
                "by_half": "ByHalfPlayerDashboard",
                "by_period": "ByPeriodPlayerDashboard",
                "by_score_margin": "ByScoreMarginPlayerDashboard",
                "by_actual_margin": "ByActualMarginPlayerDashboard",
            }
        )
    )
